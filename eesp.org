...
#+RFC_CATEGORY: std
#+RFC_NAME: draft-ietf-ipsecme-eesp
#+RFC_VERSION: 03
...
for the IPsec peer. The ~Peer Header~ is only meaningful to the
IPsec peers.
...
The overall length of all Options is limited to 255 bytes by the
...
Crypt Offset 8 (8x4 = 32 bytes). [[eesp-aad-crypt-offset-example]]
...
# AA Describe this as an exammple : packets of distinct sub SA
# AA flows(layer 4 flow) should be handled by same  network queue or cpu
# AA add text to processing Seesion ID as Sub SA
Additionally, when multiple Sub SAs exist, the receiver can
...
the SSKDF is therefore RECOMMENDED.
